...
class SCCAdapter extends utils.Adapter {
    ...
    /** State-Wert als Leistung in W parsen (Zahl, String oder Objekt mit val/powerW). */
    _parsePowerW(val) {
        if (val == null) return null;
        if (typeof val === 'number' && !isNaN(val)) return Math.max(0, val);
        if (typeof val === 'string') {
            const n = parseFloat(val);
            return !isNaN(n) ? Math.max(0, n) : null;
        }
        if (typeof val === 'object' && val !== null) {
            const v = val.val ?? val.powerW ?? val.power ?? val.W ?? val.watt;
    ...
    async onReady() {
        ...
        try {
            ...
            this.config = {
                ...
                heatPumpEnabled: config.heatPumpEnabled === true,
                heatPumpStateId: this._toStateIdString(config.heatPumpStateId) || '',
                wallboxEnabled: config.wallboxEnabled === true,
                wallboxStateId: this._toStateIdString(config.wallboxStateId) || ''
            };
    ...
    onMessage(obj) {
        ...
        const collect = async () => {
            ...
            try {
                ...
                const consumptionWallboxW = await this.getStateAsync(prefix + 'consumption.wallboxW');
                const consumptionHeatPumpW = await this.getStateAsync(prefix + 'consumption.heatPumpW');
                const consumptionWallboxEnabled = await this.getStateAsync(prefix + 'consumption.wallboxEnabled');
                const consumptionHeatPumpEnabled = await this.getStateAsync(prefix + 'consumption.heatPumpEnabled');
                ...
                data.consumption.wallboxW = consumptionWallboxW && consumptionWallboxW.val != null ? Number(consumptionWallboxW.val) : 0;
                data.consumption.heatPumpW = consumptionHeatPumpW && consumptionHeatPumpW.val != null ? Number(consumptionHeatPumpW.val) : 0;
                data.widgetOptions = {
                    wallboxEnabled: !!(consumptionWallboxEnabled && consumptionWallboxEnabled.val === true),
                    useHeatPumpImage: !!(consumptionHeatPumpEnabled && consumptionHeatPumpEnabled.val === true)
                };
    ...
    _startStandaloneServer(port) {
        ...
        const server = http.createServer(async (req, res) => {
            ...
            const safe = ['flow.html', 'admin.css', 'house.png', 'house_heatpump2.png', 'logo.png'].includes(file) ? file : null;
            if (!safe) {
                if (url === '/') {
    ...
    async ensureObjects() {
        ...
        await create(prefix + 'consumption.wallboxW', {
            type: 'number', role: 'value.power', name: 'Wallbox Leistung', unit: 'W', read: true, write: false
        });
        await create(prefix + 'consumption.heatPumpW', {
            type: 'number', role: 'value.power', name: 'Wärmepumpe Leistung', unit: 'W', read: true, write: false
        });
        await create(prefix + 'consumption.wallboxEnabled', {
            type: 'boolean', role: 'indicator', name: 'Wallbox im Flow', read: true, write: false
        });
        await create(prefix + 'consumption.heatPumpEnabled', {
            type: 'boolean', role: 'indicator', name: 'Wärmepumpe im Flow', read: true, write: false
        });
    ...
    async tick() {
        ...
        try {
            ...
            let wallboxW = 0;
            let heatPumpW = 0;
            if (this.config.wallboxEnabled && this.config.wallboxStateId) {
                try {
                    const st = await readForeign.call(this, this.config.wallboxStateId);
                    const w = st && st.val != null ? this._parsePowerW(st.val) : null;
                    if (w != null) wallboxW = w;
                } catch (e) { /* ignore */ }
            }
            if (this.config.heatPumpEnabled && this.config.heatPumpStateId) {
                try {
                    const st = await readForeign.call(this, this.config.heatPumpStateId);
                    const w = st && st.val != null ? this._parsePowerW(st.val) : null;
                    if (w != null) heatPumpW = w;
                } catch (e) { /* ignore */ }
            }
            if (!this._consumptionDeviceStatesEnsured) {
                try {
                    const ids = [
                        [prefix + 'consumption.wallboxW', { type: 'number', role: 'value.power', name: 'Wallbox Leistung', unit: 'W', read: true, write: false }],
                        [prefix + 'consumption.heatPumpW', { type: 'number', role: 'value.power', name: 'Wärmepumpe Leistung', unit: 'W', read: true, write: false }],
                        [prefix + 'consumption.wallboxEnabled', { type: 'boolean', role: 'indicator', name: 'Wallbox im Flow', read: true, write: false }],
                        [prefix + 'consumption.heatPumpEnabled', { type: 'boolean', role: 'indicator', name: 'Wärmepumpe im Flow', read: true, write: false }]
                    ];
                    for (const [id, common] of ids) {
                        const obj = await this.getObjectAsync(id);
                        if (!obj) await this.extendObjectAsync(id, { type: 'state', common });
                    }
                    this._consumptionDeviceStatesEnsured = true;
                } catch (e) {
                    this.log.debug('ensure consumption device states: ' + (e && e.message ? e.message : ''));
                }
            }
            await this.setStateAsync(prefix + 'consumption.wallboxW', this.config.wallboxEnabled ? wallboxW : 0, true);
            await this.setStateAsync(prefix + 'consumption.heatPumpW', this.config.heatPumpEnabled ? heatPumpW : 0, true);
            await this.setStateAsync(prefix + 'consumption.wallboxEnabled', !!this.config.wallboxEnabled, true);
            await this.setStateAsync(prefix + 'consumption.heatPumpEnabled', !!this.config.heatPumpEnabled, true);
            const flowPayload = {
                surplus: { powerW: surplusW, availableForDevicesW: availableW, feedInW: feedInW, priority: this.config.surplusPriority },
                consumption: {
                    totalW: totalConsumption,
                    wallboxW: this.config.wallboxEnabled ? wallboxW : 0,
                    heatPumpW: this.config.heatPumpEnabled ? heatPumpW : 0
                },
                ...
                widgetOptions: { useHeatPumpImage: !!this.config.heatPumpEnabled, wallboxEnabled: !!this.config.wallboxEnabled },
                batteries: {
